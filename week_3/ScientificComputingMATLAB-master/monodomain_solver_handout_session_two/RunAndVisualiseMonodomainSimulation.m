function [  times ] = RunAndVisualiseMonodomainSimulation( numberCells, simulationDuration, leftHalfFibreConductivity, rightHalfFibreConductivity, visualiseSimulation)
...
%
%  [finalVoltage, conductionVelocities, times] =
...
%  at the end of the simulation (finalVoltage), a vector
%  with the conduction velocity computed at each end of the fibre
%  (conductionVelocities), and a vector with the times spent in each of the
%  simulation stages (times).
%
%  An vector of times is also returned with the execution time for each of
%  the 7 simulation stages
...
firstCellVoltage = zeros(1,numberTimeSteps);
lastCellVoltage = zeros(1,numberTimeSteps);
