...
parfor cellIndex = 1:problemConfiguration.numberCells
    % Compute the ionic current I(apCellModelsState_{n-1}, V_{n-1}). apCellModelsState_{n} means apCellModelsState at time t_{n}
    ionicCurrents(cellIndex,1) = luo_rudy_1991_iionic(apCellModelsState(:,cellIndex));
    ...
    for odeSolveIndex= 1:problemConfiguration.odeSolvesPerPdeTimeStep
        apCellModelsState(:,cellIndex) = apCellModelsState(:,cellIndex) + (odeTimeStep)*luo_rudy_1991_time_deriv(currentTime, ...
