...
a_h    = [0.5/R_pool, 0.5/R_pool]; % ~0.5 inits/sec (5 to 58 init a min) based on R_pool
...
N_h    = [300.0,0.0];      % Steady-state mRNA count (this is varied)
t_e    = 17.0;       % Average 340aa gene ~ 20 aa/sec
t_h    = [17, 17];       % Average 340aa gene ~ 20 aa/sec
s_h    = [340/9,340/9];      % Average 340 codons, ribosome ~9 codons footprint
...
figure;
plot(sol.x,sol.y);
title({'Ribosome Allocation Model'});
xlabel('time, t (seconds)');
ylabel('Ribosome Count');
legend('R_{e}','R_{h1}','R_{h2}','Location', 'NorthEast');
